...
export default {
  name: "FlexBox",
  props: ["views", "edit", "centerCom", "currentCom", "pattern"],
  data() {
    return {
      center: false,
      showButton: true,
  ...
  methods: {
    ...
    focusChild(i) {
      ...
      view.focus = true;
      // 全局相对寻址
      this.$bus.emit("refreshCurrentViews", {
        views: this.views.children,
        index: i,
      });
    ...
    dropParent(e) {
      ...
      var newData =
        this.centerCom === true ? this.dropCenterCom(e) : this.dropCommon(e);
